...
% note that it's updateTaskMessage(t, b[t]()) in updateTimeStep
for t = 1:num_steps-1 % TODO code says it should be just num_steps but we'll see (wait actually how long are the x and u trajectories? num_steps or num_steps+1?)
    xhat_t = xhat{t};
    ...
    % sys->setx(xhat[t]);
    % x{it+1}(:, t) = xhat{t};
    % TODO adding optimal control and forward simulation
    u{it+1}(:, t) = -inv(R + B' * inv(Vinv{t+1}) * B) * B' * (inv(Vinv{t+1}) * (A * x{it}(:, t) + a) - v{t+1});
    x{it+1}(:, t+1) = x{it+1}(:, t) + dt * (A * x{it+1}(:, t) + a + B * u{it+1}(:, t));
end
